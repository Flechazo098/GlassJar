...
htmlSummary added removed modified total =
  let card cls count label =
        B.fromText "    <div class=\"summary-card "
          <> B.fromText cls
          <> B.fromText "\">\n"
          <> B.fromText "      <div class=\"count\">"
          <> B.fromText (T.pack $ show count)
          <> B.fromText "</div>\n"
          <> B.fromText "      <div class=\"label\">"
          <> B.fromText label
          <> B.fromText "</div>\n"
          <> B.fromText "    </div>\n"
...
htmlDiffCard i d =
  let (cssClass, badgeClass, badgeLabel) = case diffType d of
   ...
   in B.fromText "    <section id=\""
        <> B.fromText entryId
        <> B.fromText "\" class=\"entry "
        <> B.fromText cssClass
        <> B.fromText "\">\n"
        <> B.fromText "      <div class=\"entry-head\">\n"
        <> B.fromText "        <div class=\"head-left\"><span class=\"badge "
        <> B.fromText badgeClass
        <> B.fromText "\">"
        <> B.fromText badgeLabel
        <> B.fromText "</span><a class=\"entry-path entry-path-link\" href=\"?focus="
        <> B.fromText entryId
        <> B.fromText "\" target=\"_blank\" rel=\"noopener\">"
        <> B.fromText escapedName
        <> B.fromText "</a></div>\n"
        <> B.fromText "        <div class=\"head-right\">\n"
        <> renderChangeStats addCount delCount
        <> B.fromText "          <span class=\"hash-pill hash-old\">old "
        <> B.fromText oldHash
        <> B.fromText "</span>\n"
        <> B.fromText "          <span class=\"hash-pill hash-new\">new "
        <> B.fromText newHash
        <> B.fromText "</span>\n"
        <> B.fromText "        </div>\n"
        <> B.fromText "      </div>\n"
...
renderChangeStats addCount delCount =
  B.fromText "          <span class=\"diff-stats\">"
    <> B.fromText "<span class=\"stat-add\">+"
    <> B.fromText (T.pack (show addCount))
    <> B.fromText "</span>"
    <> B.fromText "<span class=\"stat-del\">-"
    <> B.fromText (T.pack (show delCount))
    <> B.fromText "</span>"
...
renderAddedRemovedTextView isAdded txt =
  let sideText = if isAdded then "Added content" else "Removed content"
   ...
   in B.fromText "<div class=\"code-pane one-side-wrapper\"><div class=\"one-side-title\">"
        <> B.fromText sideText
        <> B.fromText "</div><div class=\"one-side-grid\">"
...
renderOneSideRow cls (n, t) =
  let cellCls = diffCellClass cls
   in B.fromText "<div class=\"row one-side-row\">"
        <> B.fromText "<div class=\"ln "
        <> B.fromText cellCls
        <> B.fromText "\">"
        <> B.fromText (T.pack (show n))
        <> B.fromText "</div>"
        <> B.fromText "<div class=\"code "
        <> B.fromText cellCls
        <> B.fromText "\">"
        <> B.fromText (htmlEscape t)
        <> B.fromText "</div>"
        <> B.fromText "</div>"
...
renderSplitRow r =
  B.fromText "<div class=\"row split-row\">"
    <> B.fromText "<div class=\"ln "
    <> B.fromText (diffCellClass (rowOldClass r))
    <> B.fromText "\">"
    <> B.fromText (renderLineNo (rowOldNo r))
    <> B.fromText "</div>"
    <> B.fromText "<div class=\"code "
    <> B.fromText (diffCellClass (rowOldClass r))
    <> B.fromText "\">"
    <> B.fromText (renderCodeText (rowOldText r))
    <> B.fromText "</div>"
    <> B.fromText "<div class=\"ln "
    <> B.fromText (diffCellClass (rowNewClass r))
    <> B.fromText "\">"
    <> B.fromText (renderLineNo (rowNewNo r))
    <> B.fromText "</div>"
    <> B.fromText "<div class=\"code "
    <> B.fromText (diffCellClass (rowNewClass r))
    <> B.fromText "\">"
    <> B.fromText (renderCodeText (rowNewText r))
    <> B.fromText "</div>"
    <> B.fromText "</div>"
...
renderUnifiedRow (mNo, cls, txt) =
  let cellCls = diffCellClass cls
   in B.fromText "<div class=\"row unified-row\">"
        ...
        <> B.fromText cellCls
        <> B.fromText "\">"
        <> B.fromText (renderLineNo mNo)
        <> B.fromText "</div>"
        <> B.fromText "<div class=\"code "
        <> B.fromText cellCls
        <> B.fromText "\">"
        <> B.fromText (htmlEscape txt)
        <> B.fromText "</div>"
        <> B.fromText "</div>"
